...
import { canonicalUrl, dbPath, findWatch, latest, load, newId, nowIso, pctChange, previous, save, } from "./store.js";
...
/* ---------------- price_check ---------------- */
server.registerTool("price_check", {
    title: "Check a price now",
    description: "Fetch a product page right now and report its price, currency and title, plus the change against the last price stored for that URL. Does not create a watch.",
...
server.registerTool("watch_add", {
    title: "Watch a price",
    description: "Start tracking a product page. Fetches it once and stores the first observation. Optionally set a target price to be alerted about.",
    inputSchema: {
    ...
    if (target_price !== undefined) {
        ...
        if (!target)
            return fail(`could not read "${target_price}" as a price. Use a plain number such as 199.99`);
    }
    try {
        const o = await observe(url);
...
/* ---------------- watch_refresh ---------------- */
server.registerTool("watch_refresh", {
    title: "Refresh prices",
    description: "Re-fetch one watch or every watch, append the new observations and show current, previous, min, max, change % and target hits.",
    inputSchema: {
...
/* ---------------- price_add_manual ---------------- */
server.registerTool("price_add_manual", {
    title: "Record a price by hand",
    description: "Store a price you read yourself, for shops that block automated requests. Creates the watch if it does not exist yet.",
    inputSchema: {
        url: z.string().describe("Product page URL"),
    ...
    if (!w.currency && cur)
        w.currency = cur;
    const o = { ts: nowIso(), price: p, currency: cur ?? w.currency, source: "manual" };
...
/* ---------------- alerts_pending ---------------- */
server.registerTool("alerts_pending", {
    title: "Pending alerts",
    description: `Show watches whose latest price is at or below the target, or which dropped ${DROP_ALERT_PCT}% or more against the previous observation.`,
    inputSchema: {},
}, async () => {
    if (!gate.isPro())
        return text(gate.upgradeText("alerts_pending"));
    const db = load();
    const alerts = [];
    ...
    if (!alerts.length)
        return text("No pending alerts. Run watch_refresh first if the prices are stale.");
