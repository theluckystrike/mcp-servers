...
import { compile, compilePredicate, ExprError } from "./expr.js";
import { FREE_MAX_BYTES, FREE_MAX_ROWS, FREE_WRITE_ROWS, UserError, colLetter, expandPath, guessHeaderRow, headerNames, inferType, loadWorkbook, outputPath, parseRange, renderTable, toTable, } from "./sheet.js";
...
function capRows(rows) {
    if (gate.isPro() || rows.length <= FREE_WRITE_ROWS)
        return { rows };
    return {
...
function writeMatrix(file, headers, rows, sheetName = "Sheet1") {
    ...
    if (ext === ".csv" || ext === ".txt")
        ...
        writeAtomic(file, JSON.stringify(rows.map((r) => Object.fromEntries(headers.map((h, i) => [h, r[i] ?? null]))), null, 2));
    else {
        const ws = XLSX.utils.aoa_to_sheet(aoa);
        const wb = XLSX.utils.book_new();
        XLSX.utils.book_append_sheet(wb, ws, sheetName.slice(0, 31) || "Sheet1");
        writeAtomic(file, XLSX.write(wb, { type: "buffer", bookType: "xlsx" }));
    }
}
...
function normaliseRows(rows, existingHeaders) {
    ...
    if (Array.isArray(first)) {
        ...
        if (existingHeaders)
            return { headers: existingHeaders, matrix };
...
server.registerTool("sheet_read", {
    ...
    if (fmt === "json")
        return withNotes(o.notes, JSON.stringify(rows.map((r) => Object.fromEntries(headers.map((h, i) => [h, r[i] ?? null]))), null, 2));
    if (fmt === "csv")
...
server.registerTool("sheet_query", {
    ...
    description: "Filter, pick columns and sort rows without writing any code. where uses a small safe expression language: comparisons = != > >= < <= plus contains / startswith / endswith, combined with AND, OR, NOT and parentheses. " +
...
}, guard(async ({ path, sheet, where, select, sort, limit, as }) => {
    const o = open(path, sheet);
    ...
    if (sort) {
        ...
        if (!key)
            throw new UserError(`sort column ${JSON.stringify(sort.col)} not found. Columns: ${o.table.headers.join(", ")}`);
    ...
    const fmt = as ?? "table";
    if (fmt === "json")
        return withNotes(o.notes, JSON.stringify(shown.map((r) => Object.fromEntries(headers.map((h) => [h, r[h] ?? null]))), null, 2));
    if (fmt === "csv")
        return withNotes(o.notes, toCsv([headers, ...rows]));
...
server.registerTool("sheet_stats", {
    ...
    const out = wanted.map((h) => {
        ...
        if (nums.length && nums.length >= nonEmpty.length * 0.6) {
            ...
            if (lens.length) {
                res.min = String(nonEmpty[lens.indexOf(Math.min(...lens))]);
                res.max = String(nonEmpty[lens.indexOf(Math.max(...lens))]);
            }
        }
        return res;
    });
...
server.registerTool("sheet_write", {
    title: "Write rows",
    description: "Write rows to a spreadsheet. rows may be objects (keys become headers) or arrays (first array is the header row). " +
...
server.registerTool("sheet_add_column", {
    title: "Add a column",
    description: 'Add a computed column and save the result to a NEW file (the source is never modified unless out_path points at it). ' +
...
}, guard(async ({ path, sheet, name, formula, values, out_path }) => {
    if (!formula && !values)
    ...
    if (formula) {
        ...
        computed = recs.map((r) => {
            ...
            if (typeof v === "boolean")
                return v;
            if (typeof v === "number")
                return Number.isFinite(v) ? Number(v.toFixed(10)) : null;
            return String(v);
        });
    }
    else {
