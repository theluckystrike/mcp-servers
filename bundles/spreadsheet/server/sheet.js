...
export function requireExisting(p) {
    const full = expandPath(p);
    if (!existsSync(full))
        throw new UserError(`file not found: ${full}`);
    const st = statSync(full);
    if (st.isDirectory())
        throw new UserError(`${full} is a directory, not a spreadsheet file`);
...
function trimTrailing(m) {
    let end = m.length;
    while (end > 0 && m[end - 1].every((c) => c === null))
        end--;
...
export function loadWorkbook(pathIn, opts = {}) {
    ...
    try {
        ...
        throw new UserError(`could not open ${full} as a spreadsheet (${e.message}). Supported: .xlsx .xlsm .xlsb .xls .ods .csv .tsv`);
    }
    ...
    return {
        ...
        get(sheet) {
            ...
            if (!sheetNames.includes(name))
                throw new UserError(`sheet ${JSON.stringify(name)} not found. Sheets: ${sheetNames.map((s) => JSON.stringify(s)).join(", ")}`);
            const ws = wb.Sheets[name];
            const raw = XLSX.utils.sheet_to_json(ws, { header: 1, defval: null, blankrows: false, raw: true });
...
export function guessHeaderRow(m) {
...
export function headerNames(m, headerRow) {
    ...
    for (let c = 0; c < width; c++) {
        ...
        if (prev !== undefined) {
            seen.set(n.toLowerCase(), prev + 1);
            n = `${n}_${prev + 1}`;
        }
        else
            seen.set(n.toLowerCase(), 0);
        names.push(n);
    }
    return names;
}
...
export function toTable(ls) {
    const hr = guessHeaderRow(ls.matrix);
    const headers = headerNames(ls.matrix, hr);
    const rows = ls.matrix.slice(hr + 1);
    const width = headers.length;
    const padded = rows.map((r) => { const out = r.slice(0, width); while (out.length < width)
        out.push(null); return out; });
    return {
        headers, headerRow: hr, rows: padded,
        records() { return padded.map((r) => { const o = {}; headers.forEach((h, i) => { o[h] = r[i] ?? null; }); return o; }); },
    };
}
...
export function parseRange(range) {
    ...
    if (!m)
        throw new UserError(`range ${JSON.stringify(range)} is not A1 notation (examples: A1:D50, B2)`);
    const col = (s) => { let n = 0; for (const ch of s.toUpperCase())
        n = n * 26 + (ch.charCodeAt(0) - 64); return n - 1; };
...
export function outputPath(input, outPath, suffix, ext) {
    if (outPath)
