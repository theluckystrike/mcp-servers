...
function windowFor(from, to, pro) {
    ...
    if (pro)
        return { fromMs, toMs, clamped: false, unread: false };
...
function windowTail(w, pro) {
    if (pro)
        return "";
    if (w.unread)
        return UNREAD_WINDOW_NOTE;
...
server.registerTool("timer_status", {
    ...
    const w = { fromMs: todayStart, toMs: tomorrowStart - 1, clamped: false, unread: false };
...
server.registerTool("entry_list", {
    ...
    if (rows.length === 0) {
        ...
        if (!pro && w.unread)
            return ok(UNREAD_WINDOW_NOTE.replace(/^\n\n/, ""));
