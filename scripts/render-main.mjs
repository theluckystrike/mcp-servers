...
<tr><td>Where recommendations are sourced</td><td>${esc(l33.citation_sources.table)}</td></tr>
<tr><td>GitHub, in detail</td><td>${esc(l33.citation_sources.github_detail)}</td></tr>
<tr><td>Glama, in detail</td><td>${esc(l33.citation_sources.glama_detail)}</td></tr>
<tr><td>Retrieved but not cited</td><td>${esc(l33.retrieval_vs_citation.finding)}</td></tr>
<tr><td>Repo front door</td><td>${esc(l33.repo_front_door.clones_vs_views)}. ${esc(l33.repo_front_door.referrers)}</td></tr>
</table></div>
<p class="dim"><b>Why the one retrievable page is never cited.</b> ${esc(l33.retrieval_vs_citation.mechanism)}</p>
...
${l33.new_servers ? `<p class="dim"><b>Two servers, chosen by arithmetic.</b> ${esc(l33.new_servers.method)} ${esc(l33.new_servers.top5)} ${esc(l33.new_servers.built)} ${esc(l33.new_servers.honesty)} ${esc(l33.new_servers.not_sold_yet)}</p>` : ""}
${l33.product_gap ? `<p class="dim"><b>A product gap the measurement found.</b> ${esc(l33.product_gap.finding)} ${esc(l33.product_gap.why_it_matters)}</p>` : ""}
...
<p class="dim">${esc(l30.namespace.how)}. Scope: ${esc(l30.namespace.scope_correction)}</p>
<p class="dim"><b>Also this loop.</b> ${esc(l30.distribution.awesome_mcp_servers)} ${esc(l30.distribution.goose)}. ${esc(l30.inbound_links.new)} new inbound links on ${esc(l30.inbound_links.sites.join(", "))}, ${esc(l30.inbound_links.verified)}. Storefront at ${esc(l30.storefront.sitemap)} URLs with ${esc(l30.storefront.new_reference_pages)} new reference pages and a privacy page.</p>
<p class="dim"><b>Found elsewhere and not fixed:</b> ${esc(l30.damage_found_elsewhere.site)} lost ${esc(l30.damage_found_elsewhere.what)}. Cause: ${esc(l30.damage_found_elsewhere.cause)}. Recoverable: ${esc(l30.damage_found_elsewhere.recoverable)}</p></div>` : ""}
