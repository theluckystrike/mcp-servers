...
function tryAcquire(lockPath: string): boolean {
  try {
    ...
    if (e && e.code === "ENOENT") {
      ...
      try { mkdirSync(lockPath); return true; } catch { return false; }
    }
    throw e;
  }
}
...
function reapStale(lockPath: string): boolean {
  try {
...
export async function withFileLock<T>(
  ...
  for (;;) {
    if (tryAcquire(lockPath)) { held = true; break; }
    if (reapStale(lockPath) && tryAcquire(lockPath)) { held = true; break; }
  ...
  try {
    ...
    if (held) { try { rmdirSync(lockPath); } catch { /* already released */ } }
  }
}
