...
import { normaliseText, type Run, type Template } from "./purchase_requisition.js";

/**
 * Checklists live in this server's OWN data directory,
 * `${XDG_DATA_HOME:-~/.local/share}/mcp-servers/purchase-requisition/`, in `templates.json`,
 ...
 * servers' `dataDir()` creates its directory as a side effect of a read, and a purchase-requisition run
 ...
 * already signed. See src/purchase-requisition.ts for why that is the central rule of this server.
 *
 ...
 * "no purchase-requisitions" and letting a second run be started against a job already signed off.
 */

export function dataDir(): string {
  const base = process.env.XDG_DATA_HOME || join(homedir(), ".local", "share");
  const dir = join(base, "mcp-servers", "purchase-requisition");
...
export function findTemplate(list: Template[], ref: string): Template | undefined {
  ...
  if (pool.length > 1) {
    throw new Error(`"${ref}" matches more than one purchase-requisition: ${pool.map((t) => `${t.id} (${t.name})`).join(", ")}. Pass the exact id.`);
  }
  return pool[0];
}

export function resolveTemplate(list: Template[], ref: string): Template {
  if (!list.length) throw new Error("there is no purchase-requisition yet. Run purchase-requisition_create with a name, then purchase-requisition_item_add for each step.");
  const t = findTemplate(list, ref);
  if (!t) throw new Error(`no purchase-requisition matches "${ref}". Ids look like CL-0001. Run purchase-requisition_list to see them.`);
  return t;
}
...
export function resolveRun(list: Run[], ref: string): Run {
  if (!list.length) throw new Error("there is no purchase-requisition run yet. Run run_start with a purchase-requisition and a title.");
