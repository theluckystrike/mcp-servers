...
import type { Asset } from "./maintenance.js";

/**
 * Assets live in this server's OWN data directory,
 * `${XDG_DATA_HOME:-~/.local/share}/mcp-servers/maintenance-log/`, in `assets.json` and
 * `counter.json`. Nothing else is written anywhere.
 *
 * A read or parse failure must never be reported as "no assets": the next mutation would
 * then overwrite a register that is still on disk. Only ENOENT means empty. A parse
...
export function dataDir(): string {
  ...
  const dir = join(base, "mcp-servers", "maintenance-log");
...
function read<T>(file: string, empty: T): T {
  ...
  try {
    ...
    process.stderr.write(`mcp-maintenance-log: ${p} is not valid JSON (${(e as Error).message}); moved to ${moved}\n`);
...
/**
 * Allocate the next id in the series: `AST-<YYYY>-<NNNN>`.
 *
 * The counter is written BEFORE the asset is stored, so a crash burns a number rather
 * than reusing one. Ids already in the store are also scanned, so a restored or
 * hand-edited store cannot reissue a number that is already painted on a machine plate.
 */
...
/**
 * Resolve an asset by exact id (case-insensitive), then by exact tag, then by exact
 * name, then -- only if nothing exact matched -- by partial name. More than one partial
 * candidate is refused with the list rather than silently picking the first, so a
 * service entry cannot land on the wrong machine.
 */
export function findAsset(list: Asset[], ref: string): Asset | undefined {
  const needle = String(ref).trim().toLowerCase();
  const byId = list.find((a) => a.id.toLowerCase() === needle);
  if (byId) return byId;
  const byTag = list.filter((a) => a.tag !== null && a.tag.toLowerCase() === needle);
...
export function resolveAsset(list: Asset[], ref: string): Asset {
  const a = findAsset(list, ref);
  if (!a) throw new Error(`no asset matches "${ref}". Known: ${list.map((x) => `${x.id} (${x.name})`).join(", ") || "none"}. Nothing was written.`);
  return a;
}
