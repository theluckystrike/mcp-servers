// GENERATED by remote/build-vendor.mjs from servers/asset-register/src/index.ts. Do not edit.
...
server.registerTool("asset_journal", {
  ...
  try {
    ...
    return json({
      ...
      payloads,
      how: "Pass each payload's `arguments` to expense_add on your https://mcp.zovo.one/mcp/expense-tracker endpoint, one call per payload. `amount` is in MAJOR units because that is what expense_add takes; the minor-unit figures are on the lines above.",
      why_not_written:
...
server.registerResource("categories", "assets://categories", {
  ...
  contents: [{
    ...
    text: JSON.stringify({
      ...
      data_dir: "not a directory on this endpoint: the register is one document held per token, and asset_list reads it back",
      home: home().how,
    }, null, 2),
  }],
}));
