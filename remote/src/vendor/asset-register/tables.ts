 ...
 * as the exact bytes of servers/asset-register/src/tables/*.json, and never fetched.
...
export function table(id: TableId): Table {
  ...
  if (!raw) throw new Error(`depreciation table ${id} was not inlined into tables-data.ts`);
