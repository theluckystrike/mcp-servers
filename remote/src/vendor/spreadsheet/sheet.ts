import { ctx } from "../../shims/ctx.js";
import { closeSync, existsSync, openSync, readSync, statSync, readFileSync } from "../../shims/fs.js";
...
export function requireExisting(p: string): string {
  ...
  if (!existsSync(full)) throw new UserError(`no sheet is loaded under that name (${full.replace("/sheets/", "")}). Load one first with sheet_load {name, csv} or sheet_load {name, xlsx_base64}, and sheet_files lists what is loaded.`);
