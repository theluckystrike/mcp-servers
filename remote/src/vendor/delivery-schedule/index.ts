// GENERATED by remote/build-vendor.mjs from servers/delivery-schedule/src/index.ts. Do not edit.
...
server.registerTool("delivery_schedule_document", {
  ...
  try {
    ...
    if (businessMissing()) notes.push(`No business profile yet, so the document is headed "${PLACEHOLDER_ISSUER}". Run business_set {name, address} on your https://mcp.zovo.one/mcp/invoice endpoint once.`);
...
server.registerTool("milestone_payload", {
  ...
  try {
    ...
    const notes: string[] = [];
    if (source === "none") notes.push("The shared business profile carries no default_tax_rate and no deliverable carried its own, so VAT is 0% on those lines. Run business_set {default_tax_rate} on your https://mcp.zovo.one/mcp/invoice endpoint, or pass tax_rate.");
