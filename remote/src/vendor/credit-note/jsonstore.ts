 ...
 * empty. A parse failure quarantines the file byte-for-byte as <file>.corrupt-<timestamp>,
 * writes a marker so every later call (read or write) keeps failing until a human
 * resolves it, and throws.
 */
...
/** The marker is read by a model as often as by a human, so it explains itself. */
...
export function readJsonFile<T>(file: string, empty: T): T {
  ...
  try {
    ...
    process.stderr.write(`credit-note: ${file} is not valid JSON (${(e as Error).message}); moved to ${moved}\n`);
    throw blocked(file, moved);
  }
}
