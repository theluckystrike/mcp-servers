// GENERATED by remote/build-vendor.mjs from servers/per-diem/src/index.ts. Do not edit.
  ...
  try {
    ...
    return await locked(() => {
      ...
      if (t.traveller_source === "unknown") {
        notes.push("No traveller: the shared business profile has no name and none was given. Run business_set {name} on your https://mcp.zovo.one/mcp/invoice endpoint once and every later trip carries it.");
      }
      if (!gate.isPro()) {
...
server.registerTool("trip_export", {
  ...
  try {
    ...
    return json({
      ...
      payloads,
      how: "Pass each payload's `arguments` to expense_add on your https://mcp.zovo.one/mcp/expense-tracker endpoint, one call per payload. `amount` is in MAJOR units because that is what expense_add takes; the minor-unit figures are on the trip record.",
      why_not_written:
