...
the traffic is not our CI; it is account-wide, not monorepo-specific;
...
who the 502 are. GitHub's traffic API exposes no user agent, IP or
...
a hyphenated topic is not a substitute for the word appearing in the
...
Stars matter for a human choosing
...
`<capability>-mcp` may simply be the
...
the body of each mirror README still contains
...
Queries with 31+ results: 5 of 7 in the top 30
...
The 401 is the correct answer to an unauthenticated call, and the
...
Three GETs per URL, with a known-live and a
known-dead control:
...
`glama.ai/sitemaps/mcp-tools.xml` contains
...
contain the known-live control `GongRzhe/Office-Word-MCP-Server`. So absence from
...
`scripts/apply-mirror-seo.mjs --stagger <minutes>`
...
Reasoning: the two observed lags are 33 minutes
...
22 -> 31 surfaced, 10 -> 15 rank-1. That was the MCP-server
vocabulary and the topic expansion.

It ran roughly twenty minutes after the
...
of the repository. The crawler runs daily.
...
What is verified is that the repositories now meet the
