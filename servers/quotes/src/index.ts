...
function quoteFingerprint(q: {
  client: { name: string }; currency: string; issue_date: string; valid_until: string;
  discount_percent: number; lines: ComputedLine[]; total_minor: number; notes?: string;
}): string {
...
function exportedPdf(q: Quote): string | undefined {
  if (q.exported_path && existsSync(q.exported_path)) return q.exported_path;
 ...
 */
function dependentsOf(q: Quote): string[] {
  const out: string[] = [];
  if (q.status === "accepted") out.push(`the acceptance recorded on ${q.accepted_date}`);
  if (q.invoice_number) out.push(`invoice ${q.invoice_number} in the invoice server`);
  if (q.status === "declined") {
    out.push(
      `the decline recorded on ${q.declined_date}${q.decline_reason ? ` (${q.decline_reason})` : ""}, ` +
      `which quote_report counts in the win rate`,
    );
  }
  if (q.sent_date) out.push(`the copy sent to ${q.client.name} on ${q.sent_date}`);
  const pdf = exportedPdf(q);
  if (pdf) out.push(`the exported document at ${pdf}`);
  return out;
}
...
server.registerTool("quote_create", {
  ...
  try {
    return await locked(() => {
      ...
      if (twin) {
        return fail(
          `this is quote ${twin.id} again, line for line: same client (${twin.client.name}), same ` +
      ...
      if (!gate.isPro()) {
        notes.push(
          `Free tier: ${openCount(getQuotes(), day)} of ${FREE_OPEN_QUOTES} open quotes used. ` +
...
server.registerTool("quote_send_text", {
  ...
  try {
    ...
    await locked(() => {
      ...
      if (rec && !rec.sent_date) { rec.sent_date = day; rec.updated = new Date().toISOString(); setQuotes(list); }
    });
    return ok(body);
  } catch (e) { return fail((e as Error).message); }
});
...
server.registerTool("quote_delete", {
  ...
  try {
    return await locked(() => {
      ...
      if (deps.length) {
        return fail(
          `${q.id} (${q.client.name}, ${formatMoney(q.total_minor, q.currency)}) is not a draft: ` +
...
server.registerTool("quote_pdf", {
  ...
  try {
    ...
    await locked(() => {
      const list = getQuotes();
      const rec = list.find((x) => x.id === q.id);
      if (rec) { rec.exported_path = out; rec.updated = new Date().toISOString(); setQuotes(list); }
    });
    const notes: string[] = [];
    if (businessMissing()) notes.push(NO_BUSINESS_NOTE);
