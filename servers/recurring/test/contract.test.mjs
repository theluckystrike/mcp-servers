// Contract suite for recurring. Generated shape, mechanical assertions only.
//
// Asserts the invariants of servers/recurring/SPEC.md that a test can check without
...
function client(env = {}) {
  const home = mkdtempSync(join(tmpdir(), "mcp-contract-recurring-"));
...
test("corrupt store: the garbage is quarantined byte-for-byte and nothing is overwritten", async (t) => {
  ...
  const dir = join(c.home, "data", "mcp-servers", "recurring");
  mkdirSync(dir, { recursive: true });
  writeFileSync(join(dir, "schedules.json"), GARBAGE);

  await c.init();
  const r = await c.call("schedule_create", {"client":"A","items":[{"description":"Retainer","quantity":1,"unit_price":100}],"every":"monthly","start_date":"2026-01-05"});
  ...
  const moved = readdirSync(dir).filter((f) => f.startsWith("schedules.json.corrupt-"));
  ...
  assert.equal(readdirSync(dir).includes("schedules.json"), false, "an empty store must not be written over the corrupt one");
});

test("cap: no row-capped write tool on this server", { skip: "recurring has no row cap on a written file; its free limits are counts of stored records, covered by the server's own suites" }, () => {});
