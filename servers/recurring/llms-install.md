# Installing mcp-recurring (agent instructions)
...
Server: **Recurring invoices** (@theluckystrike/mcp-recurring)
What it does: Store recurring invoice schedules and generate the invoices that are due into the mcp-invoice server's store, with PDFs, an audit log and a revenue forecast. All data stays on the local machine.
Source: https://github.com/theluckystrike/mcp-servers/tree/main/servers/recurring
License: MIT. Support: support@zovo.one

## Status of the npm package

The npm package `@theluckystrike/mcp-recurring` is not published yet. Until it is, the `npx` command below will fail with E404. Use **Alternative B - from source** further down, which is the supported path today, and keep the same client config with `"command": "node"` and the absolute path to `dist/index.js`. Everything else on this page is unchanged.
...
```sh
npx -y @theluckystrike/mcp-recurring
```
...
or call the `license_activate` tool once with the key. Keys are verified offline; nothing is sent anywhere. Keys: https://mcp.zovo.one/buy/recurring

## Alternative A - .mcpb bundle (Claude Desktop one-click)

Download `recurring.mcpb` from https://github.com/theluckystrike/mcp-servers/releases and open it, or drag it onto the Claude Desktop Extensions pane. This installs the server without editing JSON and without Node on PATH assumptions.
...
Build `servers/invoice` before `servers/recurring`: the invoicing engine is imported from it.
Then use `"command": "node", "args": ["<abs path>/mcp-servers/servers/recurring/dist/index.js"]`.

## Alternative C - Docker

```sh
docker buildx build -f servers/recurring/Dockerfile -t mcp-recurring .
```

The build context is the repository root. Run with `docker run -i --rm -v mcp-recurring-data:/root/.local/share/mcp-servers mcp-recurring`.
...
- Data location: schedules and the generation log in `${XDG_DATA_HOME:-~/.local/share}/mcp-servers/recurring/`; the invoices they generate in `${XDG_DATA_HOME:-~/.local/share}/mcp-servers/invoice/`, shared with mcp-invoice. Both servers must see the same XDG_DATA_HOME.
