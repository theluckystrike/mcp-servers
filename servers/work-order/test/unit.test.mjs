...
test("the invoice payload's totals equal computeTotals over the payload's own items", async (t) => {
...
test("the PDF is written and names the work order", async (t) => {
...
test("the board report counts statuses, this month's hours and the unbilled value", async (t) => {
  const { c } = open(t, { key: proKey() });
  await c.init();
  const id = await workedOrder(c);
  await c.call("work_order_status", { work_order: id, status: "scheduled", date: "2026-03-03" });
