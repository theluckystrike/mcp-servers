...
test("an invoiced work order takes no more lines and no second payload", async (t) => {
...
test("an empty work order has no invoice payload, and a line cannot predate the job", async (t) => {
...
test("with no shared profile the VAT rate is zero and the payload says so", async (t) => {
