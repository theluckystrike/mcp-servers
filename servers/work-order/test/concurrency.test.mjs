...
test("forty work orders raised from two processes are all stored, with forty distinct ids", async (t) => {
...
test("lines added to one order from two processes are all kept", async (t) => {
  const { box, a, b } = pair(t, { key: proKey() });
  await Promise.all([a.init(), b.init()]);
...
test("a status change racing a line does not lose either", async (t) => {
  const { box, a, b } = pair(t, { key: proKey() });
  await Promise.all([a.init(), b.init()]);
  const id = (await a.json("work_order_create", ORDER)).created.id;
