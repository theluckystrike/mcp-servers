...
# docker buildx build -f servers/catalogue/Dockerfile .
#
...
# formatMoney, roundHalfUp), mcp-billing-docs the A4 renderer used for the price list,
# mcp-quotes the timezone-aware "today", mcp-timezone the corrupt-store quarantine.
...
RUN npm install --no-audit --no-fund \
 ...
 && npm run build --workspace @theluckystrike/mcp-catalogue
...
COPY --from=build /app/servers/catalogue/package.json ./servers/catalogue/package.json
COPY --from=build /app/servers/catalogue/dist ./servers/catalogue/dist
RUN npm install --omit=dev --no-audit --no-fund --workspace @theluckystrike/mcp-catalogue --include-workspace-root=false
CMD ["node", "servers/catalogue/dist/index.js"]
