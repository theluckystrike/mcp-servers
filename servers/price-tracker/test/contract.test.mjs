// Contract suite for price-tracker. Generated shape, mechanical assertions only.
//
// Asserts the invariants of servers/price-tracker/SPEC.md that a test can check without
...
function client(env = {}) {
  const home = mkdtempSync(join(tmpdir(), "mcp-contract-price-tracker-"));
...
test("corrupt store: the garbage is quarantined byte-for-byte and nothing is overwritten", async (t) => {
  ...
  const dir = join(c.home, "data", "mcp-servers", "price-tracker");
  mkdirSync(dir, { recursive: true });
  writeFileSync(join(dir, "watches.json"), GARBAGE);

  await c.init();
  const r = await c.call("price_add_manual", {"url":"https://example.com/x","price":9.99});
  ...
  const moved = readdirSync(dir).filter((f) => f.startsWith("watches.json.corrupt-"));
  ...
  assert.equal(readdirSync(dir).includes("watches.json"), false, "an empty store must not be written over the corrupt one");
});

test("cap: no row-capped write tool on this server", { skip: "price-tracker has no row cap on a written file; its free limits are counts of stored records, covered by the server's own suites" }, () => {});
