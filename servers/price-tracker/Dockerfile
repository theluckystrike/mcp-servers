...
# docker buildx build -f servers/price-tracker/Dockerfile .
...
RUN npm install --omit=dev --no-audit --no-fund --workspace @theluckystrike/mcp-price-tracker --include-workspace-root=false
CMD ["node", "servers/price-tracker/dist/index.js"]
