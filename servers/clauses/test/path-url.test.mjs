...
test("clause_import refuses a URL instead of resolving it against cwd", async (t) => {
  const c = client();
  t.after(() => c.close());
  await c.init();
  const r = await c.call("clause_import", { path: "http://127.0.0.1:8794/clauses.md" });
  assert.equal(r.isError, true);
  assert.match(r.text, /is a URL, not a file path/);
  assert.doesNotMatch(r.text, /no such file/);
  // never leaks the server's cwd
  assert.equal(r.text.includes(process.cwd()), false);
});

test("clause_export refuses a URL for path the same way", async (t) => {
  const c = client();
  t.after(() => c.close());
  await c.init();
  const r = await c.call("clause_export", { path: "https://example.com/out.md", format: "markdown" });
  assert.equal(r.isError, true);
  assert.match(r.text, /is a URL, not a file path/);
});
