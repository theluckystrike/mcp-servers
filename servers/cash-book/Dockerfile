...
# docker buildx build -f servers/cash-book/Dockerfile .
#
# Six siblings are copied and built too, because this server is assembled from them:
# mcp-invoice supplies the invoice ledger, the money formatting and the corrupt-store
# quarantine, mcp-billing-docs the credit notes and purchase orders, mcp-deposits the
# deposit store, mcp-statement-of-account the payment reconstruction, mcp-asset-register
# the depreciation schedules, mcp-quotes the timezone-aware "today". The expense ledger
# and the bank import are read from their data files in the shared data directory, which
# publish no library. Nothing is fetched over the network.
...
RUN npm install --omit=dev --no-audit --no-fund --workspace @theluckystrike/mcp-cash-book --include-workspace-root=false
CMD ["node", "servers/cash-book/dist/index.js"]
