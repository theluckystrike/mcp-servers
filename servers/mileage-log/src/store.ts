...
import type { Rate, Trip } from "./log.js";

/**
 * Trips and rates live in this server's OWN data directory,
 * `${XDG_DATA_HOME:-~/.local/share}/mcp-servers/mileage-log/`, in `trips.json`,
 * `rates.json` and `counter.json`. Nothing else is written anywhere.
 *
 * A read or parse failure must never be reported as "no trips": the next mutation would
 * then overwrite a log that is still on disk. Only ENOENT means empty. A parse failure
 * quarantines the file byte-for-byte as `<file>.corrupt-<timestamp>`, writes a
...
export function dataDir(): string {
  ...
  const dir = join(base, "mcp-servers", "mileage-log");
...
function read<T>(file: string, empty: T): T {
  ...
  try {
    ...
    process.stderr.write(`mcp-mileage-log: ${p} is not valid JSON (${(e as Error).message}); moved to ${moved}\n`);
...
export function getRates(): Rate[] { return read<Rate[]>("rates.json", []); }
export function setRates(v: Rate[]): void { write("rates.json", v); }

/**
 * Allocate the next id in the series: `TR-<YYYY>-<NNNN>`, the year being the year of
 * the trip date, so a reconstructed 2024 trip sorts with the 2024 trips it belongs to.
 *
 * The counter is written BEFORE the trip is stored, so a crash burns a number rather
 * than reusing one. Ids already in the store are also scanned, so a restored or
 * hand-edited store cannot reissue a number that is already sitting in an export on an
 * accountant's desk.
 */
...
/** A trip is resolved by its exact id only; a log entry is never ambiguously picked. */
export function findTrip(list: Trip[], ref: string): Trip | undefined {
  const needle = String(ref).trim().toLowerCase();
  return list.find((t) => t.id.toLowerCase() === needle);
}
