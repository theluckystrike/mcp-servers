// Contract suite for expense-tracker. Generated shape, mechanical assertions only.
//
// Asserts the invariants of servers/expense-tracker/SPEC.md that a test can check without
...
function client(env = {}) {
  const home = mkdtempSync(join(tmpdir(), "mcp-contract-expense-tracker-"));
...
test("every tool description is non-empty, single-paragraph, and within the hard ceiling", async (t) => {
  ...
  assert.equal(tools.length, 14, "tool count changed; regenerate SPEC.md with scripts/gen-spec.mjs");
...
test("corrupt store: the garbage is quarantined byte-for-byte and nothing is overwritten", async (t) => {
  ...
  const dir = join(c.home, "data", "mcp-servers", "expense-tracker");
  mkdirSync(dir, { recursive: true });
  writeFileSync(join(dir, "data.json"), GARBAGE);

  await c.init();
  const r = await c.call("expense_add", {"amount":10});
...
test("cap: an export over the free 200-row cap writes no file at all", async (t) => {
  ...
  // Seed 201 expenses directly, inside the free 30-day read window.
  const dir = join(c.home, "data", "mcp-servers", "expense-tracker");
  mkdirSync(dir, { recursive: true });
  ...
  for (let i = 0; i < 201; i++) {
    expenses.push({
      id: "e" + String(i).padStart(4, "0"), date: iso(i % 25), amount_minor: 1000 + i,
      currency: "EUR", category: "Software", merchant: "Vendor", billable: true,
      created: new Date(today.getTime() - 86400000).toISOString(),
    });
  }
  ...
  const r = await c.call("expense_export", { from: iso(29), to: iso(0), format: "csv", path: out });
  assert.match(r.text, /200/, `the refusal must name the cap: ${r.text.slice(0, 300)}`);
  assert.equal(existsSync(out), false, "a refused export must leave no file behind, not a partial one");

  // The same export succeeds once it is inside the cap, so the refusal is the cap.
  const kept = JSON.parse(readFileSync(join(dir, "data.json"), "utf8"));
  kept.expenses = kept.expenses.slice(0, 200);
  writeFileSync(join(dir, "data.json"), JSON.stringify(kept, null, 2));
