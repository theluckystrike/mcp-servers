// Codex v3 #1 (P0): a corrupt store file is never read as an empty database.
...
const GARBAGE = '[{"id":"c1","name":"Acme"  <<< truncated by a crash';

test("#1 P0: garbage in clients.json is quarantined byte-for-byte and every tool refuses", async (t) => {
  const sandbox = mkdtempSync(join(tmpdir(), "mcp-inv-corrupt-"));
  const dir = join(sandbox, "data", "mcp-servers", "invoice");
  mkdirSync(dir, { recursive: true });
  writeFileSync(join(dir, "clients.json"), GARBAGE);

  const c = client(sandbox);
  t.after(() => c.close());
  await c.init();

  const add = await c.call("client_add", { name: "Beta" });
  ...
  assert.equal(readdirSync(dir).includes("clients.json"), false, "no empty client list was written over the corrupt one");

  const read = await c.call("client_list", {});
