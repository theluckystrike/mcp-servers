// Contract suite for invoice. Generated shape, mechanical assertions only.
//
// Asserts the invariants of servers/invoice/SPEC.md that a test can check without
...
function client(env = {}) {
  const home = mkdtempSync(join(tmpdir(), "mcp-contract-invoice-"));
...
test("corrupt store: the garbage is quarantined byte-for-byte and nothing is overwritten", async (t) => {
  ...
  const dir = join(c.home, "data", "mcp-servers", "invoice");
  mkdirSync(dir, { recursive: true });
  writeFileSync(join(dir, "invoices.json"), GARBAGE);

  await c.init();
  const r = await c.call("invoice_create", {"client":"A","items":[{"description":"x","quantity":1,"unit_price":10}],"issue_date":"2026-01-05"});
  ...
  const moved = readdirSync(dir).filter((f) => f.startsWith("invoices.json.corrupt-"));
  ...
  assert.equal(readdirSync(dir).includes("invoices.json"), false, "an empty store must not be written over the corrupt one");
});

test("cap: no row-capped write tool on this server", { skip: "invoice has no row cap on a written file; its free limits are counts of stored records, covered by the server's own suites" }, () => {});
