...
| `items` | object{currency, description, quantity, tax_rate, unit_price, vat, vat_rate}[] | yes | Line items. Amounts are held as integer minor units and every line is rounded first, then summed, so the printed lines can never disagree with the total. A line may carry its own currency |
