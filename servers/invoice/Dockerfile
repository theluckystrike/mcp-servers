...
# docker buildx build -f servers/invoice/Dockerfile .
...
RUN npm install --omit=dev --no-audit --no-fund --workspace @theluckystrike/mcp-invoice --include-workspace-root=false
CMD ["node", "servers/invoice/dist/index.js"]
