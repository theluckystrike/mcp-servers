...
export const PROFILE_READERS = [
  "asset-register", "bank-statement", "barcode", "calendar", "catalogue", "clauses", "currency", "docx", "expense-tracker",
...
function normTaxId(v: unknown): string {
  return String(v ?? "").toLowerCase().replace(/[\s.\-/]/g, "");
}
function clientFingerprint(c: { name: string; address?: string; email?: string; vat_id?: string }): string {
...
server.registerTool("client_add", {
  ...
  try {
    ...
    if (duplicate) {
      return fail(
        `client "${duplicate.name}" is already stored as ${duplicate.id} with exactly these details ` +
...
const CLIENT_REFERENCES: {
  product: string; file: string; label: string;
  id: (r: Record<string, unknown>) => unknown;
  refs: (r: Record<string, unknown>) => unknown[];
}[] = [
  { product: "quotes", file: "quotes.json", label: "quote",
    id: (r) => r.id, refs: (r) => [r.client_id, (r.client as { name?: unknown } | undefined)?.name] },
  ...
  { product: "billing-docs", file: "purchase-orders.json", label: "purchase order",
    id: (r) => r.id, refs: (r) => [r.supplier_client_id, (r.supplier as { name?: unknown } | undefined)?.name] },
...
function peekRows(product: string, file: string): Record<string, unknown>[] {
  try {
...
function resolveForDelete(ref: string): { client?: Client; error?: string } {
  const clients = getClients();
...
server.registerTool("client_delete", {
  ...
  try {
    ...
    if (deps.length) {
      ...
      return fail(
        `client ${target.name} (${target.id}) is still used by ${deps.length} ` +
    ...
    setClients(clients);
    return ok(
      `Deleted client ${target.name} (${target.id}). Nothing referenced it, so no document changed. ` +
