...
function money(minor: number, currency: string): string { return formatAmount(minor, currency); }
...
export async function renderInvoicePdf(
  inv: Invoice, biz: Business, outPath: string, opts: RenderOptions,
): Promise<string> {
  ...
  const issuer: string[] = [];
  if (opts.logo && biz.logo_path && existsSync(biz.logo_path) && biz.name) issuer.push(biz.name);
  if (biz.address) issuer.push(biz.address);
  if (biz.email) issuer.push(biz.email);
  if (biz.vat_id) issuer.push(`VAT ID: ${biz.vat_id}`);
  ...
  y = doc.y + 4;
  const client: string[] = [inv.client.name];
  if (inv.client.address) client.push(inv.client.address);
  if (inv.client.email) client.push(inv.client.email);
  if (inv.client.vat_id) client.push(`VAT ID: ${inv.client.vat_id}`);
  ...
  y += 16;
  const pay: string[] = [];
  if (biz.iban) pay.push(`IBAN: ${biz.iban}`);
  if (biz.bank) pay.push(`Bank: ${biz.bank}`);
  ...
  for (let i = range.start; i < range.start + range.count; i++) {
    ...
    doc.font("Helvetica").fontSize(7.5).fillColor(MUTED)
      .text(`Invoice ${inv.number}  |  page ${i - range.start + 1} of ${range.count}`,
