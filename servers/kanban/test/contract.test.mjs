// Contract suite for kanban. Mechanical assertions only: the invariants of
// servers/kanban/SPEC.md that a test can check without judgement.
...
test("a tool that takes a file path or a URL opens with an imperative sentence", async (t) => {
  ...
  // This server takes no path or URL anywhere: every tool works on the local board.
  // The assertion is that this stays true, because a new file argument would need the
  // imperative opener the other servers use.
  const fileTools = tools.filter((x) => Object.keys(x.inputSchema?.properties ?? {}).some((k) => FILE_ARGS.includes(k)));
  assert.deepEqual(fileTools.map((x) => x.name).sort(), [], "kanban must take no file path or URL");
  ...
  assert.deepEqual(offenders.filter((n) => !NON_IMPERATIVE_BASELINE.includes(n)), [],
    'new file/URL tools whose description does not start with "Call this tool" or "Use this"');
});
...
test("corrupt store: the garbage is quarantined byte-for-byte and nothing is overwritten", async (t) => {
  ...
  t.after(() => { c.close(); cleanup(box.dir); });
  await c.init();
  const r = await c.call("task_list", {});
...
test("cap: the fourth project is refused and nothing is added to the store", async (t) => {
  const box = sandbox();
  const c = client({ dataHome: box.dataHome });
  t.after(() => { c.close(); cleanup(box.dir); });
  await c.init();
  ...
  const fourth = await c.call("task_add", { project: "delta", title: "first on delta" });
  assert.match(fourth.text, /free/i, fourth.text.slice(0, 300));

  const dir = join(box.dataHome, "mcp-servers", PRODUCT);
  const db = JSON.parse(readFileSync(join(dir, "data.json"), "utf8"));
  const names = Object.keys(db.boards ?? {}).sort();
  ...
  assert.ok(fourth.text.includes(wantBundle),
    `the same cap message must carry ${wantBundle}, got: ` + fourth.text.slice(0, 600));
});
