 ...
 */
function profileCurrency(): string | undefined {
  const c = readSharedProfile().default_currency;
  return c && /^[A-Za-z]{3}$/.test(c.trim()) ? c.trim().toUpperCase() : undefined;
}

function needCurrency(explicit: string | undefined, what: string): { code: string; fromProfile: boolean } | string {
  if (explicit) return { code: norm(explicit), fromProfile: false };
  const p = profileCurrency();
  if (p) return { code: p, fromProfile: true };
