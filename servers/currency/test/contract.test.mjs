// Contract suite for currency. Generated shape, mechanical assertions only.
//
// Asserts the invariants of servers/currency/SPEC.md that a test can check without
...
function client(env = {}) {
  const home = mkdtempSync(join(tmpdir(), "mcp-contract-currency-"));
...
test("corrupt store: the garbage is quarantined byte-for-byte and nothing is overwritten", async (t) => {
  ...
  const dir = join(c.home, "data", "mcp-servers", "currency");
  mkdirSync(dir, { recursive: true });
  writeFileSync(join(dir, "daily.json"), GARBAGE);
  ...
  assert.equal(readdirSync(dir).includes("daily.json"), false, "an empty store must not be written over the corrupt one");
});

test("cap: no row-capped write tool on this server", { skip: "currency has no row cap on a written file; its free limits are counts of stored records, covered by the server's own suites" }, () => {});
