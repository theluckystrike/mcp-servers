...
| `name` | string | no | Your own name. Leave it out and the shared business profile's name is used, so you are never asked for a name the suite already holds (minLength 1) |
...
- `Error: no name: pass name, or set one once in the shared business profile (the invoice server's business_set, field name) and every server in this suite uses it. Nothing was stored.`
