...
# docker buildx build -f servers/resume/Dockerfile .
#
...
RUN npm install --no-audit --no-fund \
 ...
 && npm run build --workspace @theluckystrike/mcp-resume
...
COPY --from=build /app/servers/resume/package.json ./servers/resume/package.json
COPY --from=build /app/servers/resume/dist ./servers/resume/dist
RUN npm install --omit=dev --no-audit --no-fund --workspace @theluckystrike/mcp-resume --include-workspace-root=false
CMD ["node", "servers/resume/dist/index.js"]
