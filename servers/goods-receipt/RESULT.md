...
evidence: |
  ...
  goods-receipt snag handover onboarding` fully paginated the live registry:
  goods-receipt 6 rows / 6 servers / 5 namespaces, 6 rows sorting before
  ...
  `gh api "search/repositories?q=goods-receipt+mcp+in:name,description"` total_count 191,
  `q=goods-receipt` total_count 51379. goods-receipt is one of the few candidate tokens whose bare
  ...
  Suites, `node --test test/*.test.mjs` in servers/goods-receipt:
...
artifacts: |
  servers/goods-receipt/{src,test}, README.md, SPEC.md, llms-install.md, Dockerfile,
...
insight: |
  ...
  if it broke. A run copies its goods-receipt's steps at start and records the version it copied.
  Without that, editing a goods-receipt rewrites history: a handover certificate signed for ten
  ...
  `na` must count as ANSWERED and never as PASSED. Merging the two is how a goods-receipt reports
