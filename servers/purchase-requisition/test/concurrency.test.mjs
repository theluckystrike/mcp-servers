...
test("two processes, one run: every answer survives", async (t) => {
  const box = sandbox("mcp-purchase-requisition-conc-");
  ...
  const started = await a.json("run_start", { purchaseRequisition: cl, title: "Van Z", date: "2026-02-14" });
...
test("two processes creating purchase-requisitions and runs at once never reuse an id", async (t) => {
  const box = sandbox("mcp-purchase-requisition-ids-");
  writeProfile(box.dataHome);
  const key = proKey();
  const a = client({ dataHome: box.dataHome, key });
  const b = client({ dataHome: box.dataHome, key });
  ...
  const made = await Promise.all(Array.from({ length: 10 }, (_, i) =>
    (i % 2 === 0 ? a : b).call("purchase-requisition_create", { name: `List ${i}` })));
  const ids = made.map((r) => { assert.equal(r.isError, false, r.text); return JSON.parse(r.text).created.id; });
  assert.equal(new Set(ids).size, 10, `a purchase-requisition id was reused: ${ids.sort().join(", ")}`);
  ...
  const runs = await Promise.all(Array.from({ length: 10 }, (_, i) =>
    (i % 2 === 0 ? a : b).call("run_start", { purchaseRequisition: cl, title: `Job ${i}`, date: "2026-02-14" })));
