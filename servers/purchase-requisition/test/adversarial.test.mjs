...
test("a refusal writes nothing: both store files are byte-identical afterwards", async (t) => {
  ...
  const refusals = [
    ["purchase-requisition_create", { name: "   " }],
    ["purchase-requisition_create", { name: CHECKLIST_NAME }],                       // duplicate name
    ["purchase-requisition_item_add", { purchaseRequisition, text: " " }],
    ["purchase-requisition_item_add", { purchaseRequisition: "CL-9999", text: "X" }],
    ["purchase-requisition_item_remove", { purchaseRequisition, item: "I99" }],
    ["purchase-requisition_delete", { purchaseRequisition }],                                  // confirm missing
    ["run_start", { purchaseRequisition, title: "  " }],
    ["run_start", { purchaseRequisition: "CL-9999", title: "X" }],
...
test("a refusal says what was not written, in words a user can act on", async (t) => {
  ...
  const { purchaseRequisition, run } = await seed(c);
  const cases = [
    [["purchase-requisition_create", { name: CHECKLIST_NAME }], /already exists/],
    [["purchase-requisition_item_remove", { purchaseRequisition, item: "I42" }], /has no step "I42"/],
    ...
    [["purchase-requisition_show", { purchaseRequisition: "CL-9999" }], /no purchase-requisition matches/],
  ];
...
test("an ambiguous name is refused with the candidates, never resolved to the first", async (t) => {
  const { c } = open(t);
  await c.init();
  await c.json("purchase-requisition_create", { name: "Site handover" });
  await c.json("purchase-requisition_create", { name: "Site handover, electrical" });
  const exact = await c.call("purchase-requisition_show", { purchaseRequisition: "Site handover" });
  assert.equal(exact.isError, false, "an exact name match must win outright");
  assert.equal(JSON.parse(exact.text).purchaseRequisition.name, "Site handover");
  const ambiguous = await c.call("purchase-requisition_show", { purchaseRequisition: "handover" });
  assert.ok(ambiguous.isError, ambiguous.text);
  assert.match(ambiguous.text, /matches more than one purchase-requisition/);
...
test("an ambiguous run reference is refused too", async (t) => {
  ...
  await c.json("run_start", { purchaseRequisition: cl, title: "Van A", reference: "WO-1" });
  await c.json("run_start", { purchaseRequisition: cl, title: "Van A again", reference: "WO-1" });
...
test("text is normalised on the way in, and over-long text is refused not truncated", async (t) => {
  const { c } = open(t);
  await c.init();
  const t0 = await c.json("purchase-requisition_create", { name: "  Site   handover  ", category: "  On   Site  " });
  assert.equal(t0.created.name, "Site handover");
  assert.equal(t0.created.category, "on-site");
  const long = "x".repeat(2001);
  assert.ok((await c.call("purchase-requisition_item_add", { purchaseRequisition: t0.created.id, text: long })).isError, "a 2001-character step was accepted");
  const at = await c.json("purchase-requisition_item_add", { purchaseRequisition: t0.created.id, text: "y".repeat(2000) });
  ...
  assert.ok((await c.call("purchase-requisition_item_add", { purchaseRequisition: t0.created.id, text: "Z", section: "s".repeat(61) })).isError);
  const s = await c.json("purchase-requisition_item_add", { purchaseRequisition: t0.created.id, text: "Z", section: "s".repeat(60) });
...
test("a step with no section groups under null and prints after the named sections", async (t) => {
  ...
  await c.init();
  const cl = await seedChecklist(c);
  await c.json("purchase-requisition_item_add", { purchaseRequisition: cl, text: "Anything else noted", required: false });
  const r = await c.json("purchase-requisition_show", { purchaseRequisition: cl, as_text: true });
  assert.deepEqual(r.purchaseRequisition.sections_detail.map((g) => g.section), ["Exterior", "Load", null]);
...
test("the free-tier cap counts purchase-requisitions, not runs, and deleting one gives the slot back", async (t) => {
  const { c } = open(t);
  await c.init();
  const ids = [];
  for (let i = 1; i <= 3; i++) ids.push((await c.json("purchase-requisition_create", { name: `List ${i}` })).created.id);
  await c.json("purchase-requisition_item_add", { purchaseRequisition: ids[0], text: "Step" });
  for (let i = 0; i < 20; i++) assert.equal((await c.call("run_start", { purchaseRequisition: ids[0], title: `Job ${i}` })).isError, false);
  assert.equal((await c.json("purchase-requisition_list", {})).free_tier_purchaseRequisition_limit, 3);
  assert.ok((await c.call("purchase-requisition_create", { name: "List 4" })).isError);
  await c.json("purchase-requisition_delete", { purchaseRequisition: ids[1], confirm: true });
  assert.equal((await c.call("purchase-requisition_create", { name: "List 4" })).isError, false);
});
...
test("a purchase-requisition with 500 steps is the ceiling and the 501st is refused", async (t) => {
  ...
  const { MAX_ITEMS } = await import("../dist/purchase_requisition.js");
  assert.equal(MAX_ITEMS, 500);
  const t0 = await c.json("purchase-requisition_create", { name: "Ceiling" });
  const added = await c.json("purchase-requisition_item_add", { purchaseRequisition: t0.created.id, text: "One" });
  assert.equal(added.purchaseRequisition.items, 1);
...
test("run_list filters compose rather than replacing one another", async (t) => {
  ...
  await c.init();
  const cl = await seedChecklist(c);
  const a = await c.json("run_start", { purchaseRequisition: cl, title: "Van A", reference: "WO-1", date: "2026-02-01" });
  const b = await c.json("run_start", { purchaseRequisition: cl, title: "Van B", reference: "WO-1", date: "2026-02-02" });
...
test("a run started from a purchase-requisition that is later renamed keeps the name it saw", async (t) => {
  const { c } = open(t);
  await c.init();
  const { purchaseRequisition, run } = await seed(c);
  ...
  await c.json("purchase-requisition_delete", { purchaseRequisition, confirm: true });
  await c.json("purchase-requisition_create", { name: "Pre-delivery vehicle check v2" });
  const r = await c.json("run_show", { run });
  assert.equal(r.purchaseRequisition_name, CHECKLIST_NAME);
  assert.equal(r.purchaseRequisition, "CL-0001");
});
