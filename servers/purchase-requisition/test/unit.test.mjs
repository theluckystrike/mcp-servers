// The behaviour, against the worked purchaseRequisition and run in _client.mjs. Every count here was
...
test("a purchase-requisition keeps its steps in order, grouped by section, with the required flag", async (t) => {
  const { c } = open(t);
  await c.init();
  const id = await seedChecklist(c);
  assert.equal(id, "CL-0001");
  const r = await c.json("purchase-requisition_show", { purchaseRequisition: id });
  assert.equal(r.purchaseRequisition.items, 6);
  assert.equal(r.purchaseRequisition.required, 4);
  assert.equal(r.purchaseRequisition.sections, 2);
  assert.equal(r.purchaseRequisition.category, "pre-delivery");
  assert.deepEqual(r.purchaseRequisition.sections_detail.map((g) => g.section), ["Exterior", "Load"]);
  assert.deepEqual(r.purchaseRequisition.sections_detail[0].items.map((i) => i.id), ["I01", "I02", "I03"]);
  assert.deepEqual(r.purchaseRequisition.sections_detail[1].items.map((i) => i.id), ["I04", "I05", "I06"]);
  // The version starts at 1 and bumps once per item.
  assert.equal(r.purchaseRequisition.version, 1 + ITEMS.length);
});
...
test("a run snapshots its purchaseRequisition: editing the purchase-requisition afterwards changes nothing", async (t) => {
  const { c } = open(t);
  await c.init();
  const cl = await seedChecklist(c);
  const run = await seedRun(c, cl);
  const before = await c.json("run_show", { run });
  assert.equal(before.purchaseRequisition_version, 7);
  // Add a step and delete another one, then rename nothing and check the run is untouched.
  await c.json("purchase-requisition_item_add", { purchaseRequisition: cl, text: "Fuel card in cab", section: "Load", required: true });
  await c.json("purchase-requisition_item_remove", { purchaseRequisition: cl, item: "I01" });
  const tmpl = await c.json("purchase-requisition_show", { purchaseRequisition: cl });
  assert.equal(tmpl.purchaseRequisition.items, 6);
  assert.equal(tmpl.purchaseRequisition.version, 9);
  const after = await c.json("run_show", { run });
  assert.equal(after.progress.items, 6, "the run gained or lost a step when the purchase-requisition changed");
  assert.equal(after.purchaseRequisition_version, 7, "the run's recorded purchaseRequisition version moved");
  ...
  const fresh = await c.json("run_start", { purchaseRequisition: cl, title: "Van CX22 LMN" });
  assert.equal(fresh.started.items, 6);
  assert.equal(fresh.started.purchaseRequisition_version, 9);
...
test("deleting a purchaseRequisition leaves its runs readable and complete", async (t) => {
  const { c } = open(t);
  await c.init();
  const { purchaseRequisition, run } = await seed(c);
  const d = await c.json("purchase-requisition_delete", { purchaseRequisition, confirm: true });
  assert.equal(d.runs_kept, 1);
  const r = await c.json("run_show", { run });
  assert.equal(r.progress.items, 6);
  assert.equal(r.purchaseRequisition_name, CHECKLIST_NAME, "the run lost the name of the purchaseRequisition it came from");
  assert.equal(r.progress.pass, 3);
});
...
test("runs are listed newest first and filter by purchaseRequisition, reference, status and failures", async (t) => {
  const { c } = open(t);
  await c.init();
  const cl = await seedChecklist(c);
  await c.json("run_start", { purchaseRequisition: cl, title: "Van A", reference: "WO-1", date: "2026-02-01" });
  const b = await c.json("run_start", { purchaseRequisition: cl, title: "Van B", reference: "WO-2", date: "2026-02-05" });
  ...
  assert.equal((await c.json("run_list", { purchaseRequisition: cl })).runs.length, 2);
...
test("a step can be inserted at a position and existing ids do not move", async (t) => {
  ...
  await c.init();
  const cl = await seedChecklist(c);
  const r = await c.json("purchase-requisition_item_add", { purchaseRequisition: cl, text: "Keys accounted for", section: "Exterior", position: 1 });
  assert.equal(r.added.id, "I07", "inserting must not renumber the steps already there");
  const show = await c.json("purchase-requisition_show", { purchaseRequisition: cl });
  assert.deepEqual(show.purchaseRequisition.sections_detail[0].items.map((i) => i.id), ["I07", "I01", "I02", "I03"]);
});

test("the free tier holds three purchaseRequisitions and never caps runs", async (t) => {
  const { c } = open(t);
  await c.init();
  const ids = [];
  for (let i = 1; i <= 3; i++) ids.push(await seedChecklist(c, `List ${i}`));
  const fourth = await c.call("purchase-requisition_create", { name: "List 4" });
  assert.ok(fourth.isError, fourth.text);
  assert.match(fourth.text, /free tier holds 3 purchase-requisitions/);
  assert.match(fourth.text, /mcp\.zovo\.one\/buy\/purchase-requisition\?src=purchase-requisition\.purchase_requisition_create/);
  // Runs are not capped: twelve of them on the free tier.
  for (let i = 0; i < 12; i++) {
    const r = await c.call("run_start", { purchaseRequisition: ids[0], title: `Job ${i}` });
    assert.equal(r.isError, false, r.text);
  }
  assert.equal((await c.json("run_list", {})).total, 12);
  // Deleting a purchaseRequisition gives the slot back.
  await c.json("purchase-requisition_delete", { purchaseRequisition: ids[2], confirm: true });
  assert.equal((await c.call("purchase-requisition_create", { name: "List 4" })).isError, false);
});

test("pro lifts the purchaseRequisition cap and writes the report to a file", async (t) => {
  const { box, c } = open(t, { key: proKey() });
  await c.init();
  for (let i = 1; i <= 5; i++) {
    const r = await c.call("purchase-requisition_create", { name: `List ${i}` });
