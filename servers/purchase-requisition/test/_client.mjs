// Shared stdio JSON-RPC client for the mcp-purchaseRequisition suites.
// One sandboxed data dir per client, so no test can see another's purchaseRequisitions.
...
export function proKey(product = "purchase-requisition") {
  return execFileSync(process.execPath, [join(REPO, "scripts", "sign-license.mjs"), product], { encoding: "utf8" }).trim();
}

export function sandbox(prefix = "mcp-purchase-requisition-") {
...
export function client({ dataHome, key } = {}) {
  const home = dataHome ?? join(mkdtempSync(join(tmpdir(), "mcp-purchase-requisition-")), "data");
...
export function storeDir(dataHome) { return join(dataHome, "mcp-servers", "purchase-requisition"); }

/**
 * The worked purchaseRequisition: a pre-delivery vehicle check, six steps in two sections, four of
 * them required.
 *
...
/** Create the worked purchaseRequisition. Returns its id. */
export async function seedChecklist(c, name = CHECKLIST_NAME) {
  const r = await c.call("purchase-requisition_create", { name, category: "Pre delivery", description: "Run before every van leaves the yard." });
  if (r.isError) throw new Error(`purchase-requisition_create: ${r.text}`);
  const id = JSON.parse(r.text).created.id;
  for (const i of ITEMS) {
    const x = await c.call("purchase-requisition_item_add", { purchaseRequisition: id, ...i });
    if (x.isError) throw new Error(`purchase-requisition_item_add ${i.text}: ${x.text}`);
  }
  return id;
}

/** Start the worked run and answer five of its six steps. Returns the run id. */
export async function seedRun(c, purchaseRequisitionId, answers = ANSWERS) {
  const r = await c.call("run_start", { purchaseRequisition: purchaseRequisitionId, title: RUN_TITLE, reference: RUN_REFERENCE, date: RUN_DATE });
...
export async function seed(c) {
  ...
  return { purchaseRequisition: cl, run };
}
