...
evidence: |
  ...
  purchase-requisition snag handover onboarding` fully paginated the live registry:
  purchase-requisition 6 rows / 6 servers / 5 namespaces, 6 rows sorting before
  ...
  `gh api "search/repositories?q=purchase-requisition+mcp+in:name,description"` total_count 191,
  `q=purchase-requisition` total_count 51379. purchase-requisition is one of the few candidate tokens whose bare
  ...
  Suites, `node --test test/*.test.mjs` in servers/purchase-requisition:
...
artifacts: |
  servers/purchase-requisition/{src,test}, README.md, SPEC.md, llms-install.md, Dockerfile,
...
insight: |
  ...
  if it broke. A run copies its purchase-requisition's steps at start and records the version it copied.
  Without that, editing a purchase-requisition rewrites history: a handover certificate signed for ten
  ...
  `na` must count as ANSWERED and never as PASSED. Merging the two is how a purchase-requisition reports
